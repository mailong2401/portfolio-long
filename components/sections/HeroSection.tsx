...
function TypingRole() {
  ...
  // Lấy roles từ translations
  const roles = useMemo(() => {
    const rolesArray = [];
    let i = 0;
    while (true) {
      const role = t(`hero.roles.${i}`);
      if (role === `hero.roles.${i}`) break;
  ...
  return (
    <span className="text-normal-blue font-medium min-w-[120px] inline-block">
      {displayText}
        ...
        <ScrollAnimation direction="up" delay={0.7} duration={0.6}>
          <div className="my-8 flex items-center gap-4">
            ...
            <div className="flex items-center gap-2 px-4 py-1 bg-background/30 rounded-full border border-button-bg/30 shadow-border-md hover:shadow-border-lg transition-all duration-300 hover:scale-105 cursor-default">
              ...
              <span className="text-xs text-primary-dim-fg">{t('terminal.os').replace('OS: ', '')}</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          <ScrollAnimation direction="left" delay={0.9} duration={0.6}>
            <PassionCard
              ...
              title={t('passion.creativity')}
              description={t('passion.creativityDesc')}
          ...
          <ScrollAnimation direction="right" delay={1.0} duration={0.6}>
            <PassionCard
              ...
              title={t('passion.customUI')}
              description={t('passion.customUIDesc')}
